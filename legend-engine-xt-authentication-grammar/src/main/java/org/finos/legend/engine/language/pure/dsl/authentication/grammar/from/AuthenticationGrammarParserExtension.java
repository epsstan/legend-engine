        ...
        return Collections.singletonList(code ->
        {
            ...
            switch (code.getType())
            {
                case "PropertiesFileSecret":
                    return parseSecret(code, p -> walker.visitPropertiesFileCredentialVaultSecret(p.propertiesVaultSecret()));
                case "EnvironmentSecret":
                    return parseSecret(code, p -> walker.visitEnvironmentVaultCredentialVaultSecret(p.environmentVaultSecret()));
                case "SystemPropertiesSecret":
                    return parseSecret(code, p -> walker.visitSystemPropertiesCredentialVaultSecret(p.systemPropertiesVaultSecret()));
                default:
                    return null;
            }
        });
