                ...
                (connectionValue, context) ->
                {
                    if (connectionValue instanceof ServiceStoreConnection)
                    {
                        ...
                        if (serviceStoreConnection.authSpecs != null)
                        {
                            ...
                            pureServiceStoreConnection._authSpecs(new PureMap(HelperServiceStoreBuilder.compileAuthentication(serviceStoreConnection, pureServiceStore, context).stream().collect(Collectors.toMap(Pair::getOne, Pair::getTwo))));
                        }
