...
public class AuthenticationProtocolExtension implements PureProtocolExtension
{
    ...
    public List<Function0<List<ProtocolSubTypeInfo<?>>>> getExtraProtocolSubTypeInfoCollectors()
    {
        return Lists.fixedSize.with(() -> Lists.fixedSize.with(
                ProtocolSubTypeInfo.newBuilder(AuthenticationSpec.class)
                        .withSubtypes(FastList.newListWith(
                                Tuples.pair(UsernamePasswordAuthenticationSpec.class, "usernamePassword"),
                                Tuples.pair(ApiKeyAuthenticationSpec.class,"apiKey"),
                                Tuples.pair(OAuthAuthenticationSpec.class,"oauth")))
                        .build()
        ));
    }
}
